	...
	function createHeader(containerElement) {
		...
		createLabel(headerElement, 'Settings', SETTINGS_LABEL_TITLE_CLASSNAME);
	...
	function onButtonClick(e, callback) {
		...
		if (key == "ResetSettings") {
			showMessageDialog("Reset settings", "Do you want to reset the application?", [{ label: "Yes", commandId: 0 }, { label: "No", commandId: 1 }])
	...
	function navigate(from, to, appBarVisible) {
		...
		if (appBarElement && appBarElement.winControl) {
			appBarElement.winControl.disabled = !appBarVisible;
		}
	...
	require("cordova/windows8/commandProxy").add("AppPreferences", module.exports);
