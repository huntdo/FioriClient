cordova.define("com.sap.mp.cordova.plugins.logon.Logon", function(require, exports, module) {     var utils = sap.logon.Utils;
   ...
   //-- state: begin --> run -> done (switch to another state) --> if don't exist any configure source type then start logon init
    ...
    var startLogonInit = function(context){
        ...
        function logon(){
            ...
            if (context.appConfig.isForSMP){
               sap.Logon.init(function(m) {appDelegate.onRegistrationSuccess(m); navigator.splashscreen.hide();}, function(m) {appDelegate.onRegistrationError(m); navigator.splashscreen.hide();}, context.appConfig.appID, context.smpConfig, context.operation.logonView, context.appConfig.certificate);
            }
            else{
               ...
               sap.Logon.initPasscodeManager(function(m){appDelegate.onRegistrationSuccess(m); navigator.splashscreen.hide();}, function(m) {appDelegate.onRegistrationError(m); navigator.splashscreen.hide();}, context.appConfig.appID, context.operation.logonView, context.appConfig.passcodePolicy, context.smpConfig, context.appConfig.certificate);
            }
        }
    };
